...
class AutProxySettings(ProxySettings):
    def __init__(
        self,
        host_or_url,  # type: Text
        port=None,  # type: Optional[int]
        ...
        domains=None,  # type: Optional[List[Text]]
    ):
        # type: (...) -> None
        super().__init__(host_or_url, port, username, password, scheme)
        if mode is not None and mode not in ("Allow", "Block"):
            raise ValueError("mode must be 'Allow' or 'Block', got {!r}".format(mode))
        self.mode = mode
...
class Configuration(object):
    ...
    disable_nml_url_cache = attr.ib(default=None)  # type: Optional[bool]
    aut_proxy = attr.ib(default=None)  # type: Optional[AutProxySettings]
    ...
    def set_user_test_id(self, user_test_id):
        # type: (Text) -> Self
        argument_guard.is_a(user_test_id, str)
        self.user_test_id = user_test_id
        return self

    def set_disable_nml_url_cache(self, disable_nml_url_cache):
        # type: (bool) -> Self
        argument_guard.is_a(disable_nml_url_cache, bool)
        self.disable_nml_url_cache = disable_nml_url_cache
        return self

    def set_aut_proxy(self, aut_proxy):
        # type: (Optional[AutProxySettings]) -> Self
        if aut_proxy is not None:
            argument_guard.is_a(aut_proxy, AutProxySettings)
        self.aut_proxy = aut_proxy
        return self
